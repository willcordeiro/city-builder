...
import { createCity } from "@/utils/cityGenerator";
...
export default function CityProvider({
  size,
  children,
}: PropsWithChildren<CityProviderProps>) {
  ...
  const updateAssetLoading = useCallback(
    (x: number, y: number, isLoading: boolean) => {
      setCity((prevCity) => ({
        ...prevCity,
  ...
  const updateTileBuilding = useCallback(
